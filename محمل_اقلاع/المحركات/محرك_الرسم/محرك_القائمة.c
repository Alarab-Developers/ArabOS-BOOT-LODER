...
VOID Menu_ReadFromFile(
    ...
    CHAR16         *outSystemArchive)
{
    static const CHAR8 BG_KEY[]       = "\xd8\xa7\xd9\x84\xd8\xae\xd9\x84\xd9\x81\xd9\x8a\xd8\xa9 =";
    /* "ملف النظام =" - مسار ملف النظام المضغوط (بديل sys.tar الثابت) */
    static const CHAR8 SYSARCHIVE_KEY[] = "\xd9\x85\xd9\x84\xd9\x81 \xd8\xa7\xd9\x84\xd9\x86\xd8\xb8\xd8\xa7\xd9\x85 =";
    ...
    while (buf[pos]) {
        ...
        if (line[0] != '#' && line[0] != ';' && line[0] != '[' && line[0] != '\0') {
            ...
            else if (starts_with(line, SYSARCHIVE_KEY)) {
                CHAR8 *val = line + sizeof(SYSARCHIVE_KEY) - 1;
                trim_leading(&val);
                Utf8ToUcs2(outSystemArchive, 256, val);
            }
